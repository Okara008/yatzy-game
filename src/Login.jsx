...
function Login({getNames}) {
    ...
    async function create_user_row(name, index){
        try{
            const response = await fetch("https://yatzycore.liveblog365.com/api_insert_new_user.php",{
                method: "POST",
                headers: {
    ...
    async function validate_user_exists(name, index){
        ...
        setIsLoading(true);
        try {
            const response = await fetch("https://yatzycore.liveblog365.com/api_validate_user_exists.php", {
                method: "POST",
